...
for file_path in glob.glob(os.path.join(folder_path, '*.csv')):
    ...
    # 为top_20添加基因名和排名列
    top_20['gene_name'] = gene_name
    top_20['rank'] = range(1, 21)
    
    # 将筛选后的数据追加到all_results中
    all_results = pd.concat([all_results, top_20], ignore_index=True)

# 构建输出文件路径
output_file_path = os.path.join(output_folder, 'combined_top_20_correlations_with_rank.csv')
